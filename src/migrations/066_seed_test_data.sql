...
INSERT INTO hangar_owners (id, user_id, company_name, cnpj, bank_code, bank_agency, bank_account, account_holder_name, is_active, verified, created_at, updated_at, owner_type, cpf, pix_key, pix_key_type)
SELECT gen_random_uuid(), u.id, 'SkyHoldings Ltd', '12345678000199', '001', '1234', '123456-7', 'Joao Oliveira', true, true, NOW() - INTERVAL '120 days', NOW() - INTERVAL '120 days', 'company', '11122233344', 'joao@pix', 'email'
FROM users u WHERE u.email = 'joao.owner@email.com'
ON CONFLICT DO NOTHING;

INSERT INTO hangar_owners (id, user_id, company_name, cnpj, bank_code, bank_agency, bank_account, account_holder_name, is_active, verified, created_at, updated_at, owner_type, cpf, pix_key, pix_key_type)
SELECT gen_random_uuid(), u.id, 'Costa Aviation', '98765432000155', '237', '5678', '765432-1', 'Pedro Costa', true, false, NOW() - INTERVAL '100 days', NOW() - INTERVAL '100 days', 'company', '22233344455', 'pedro@pix', 'email'
FROM users u WHERE u.email = 'pedro.owner@email.com'
ON CONFLICT DO NOTHING;

INSERT INTO hangar_owners (id, user_id, company_name, cnpj, bank_code, bank_agency, bank_account, account_holder_name, is_active, verified, created_at, updated_at, owner_type, cpf, pix_key, pix_key_type)
SELECT gen_random_uuid(), u.id, 'Ferreira Aero', '10203040000188', '104', '9101', '234567-8', 'Ana Ferreira', true, false, NOW() - INTERVAL '90 days', NOW() - INTERVAL '90 days', 'company', '33344455566', 'ana@pix', 'email'
