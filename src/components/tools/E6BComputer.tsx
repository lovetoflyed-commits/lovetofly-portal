...
const i18n = {
  EN: {
    ...
    buttons: {
      ...
      unit: 'UNIT', conv: 'CONV', back: 'BACK', clr: 'CLR', enter: 'ENTER', calc: 'CALC'
    },
    labels: {
  ...
  PT: {
    ...
    buttons: {
      flt: 'FLT', plan: 'PLAN', timer: 'CRON', menu: 'MENU',
      wind: 'VENTO', hdg: 'PROA', tas: 'TAS', alt: 'ALT',
      unit: 'UNID', conv: 'CONV', back: 'VOLTAR', clr: 'LIMPAR', enter: 'ENTRAR', calc: 'CALC'
    },
    labels: {
...
export default function E6BComputer() {
  ...
  const [calcOperand1, setCalcOperand1] = useState<number | null>(null);
  ...
  const handlePress = (key: string) => {
    ...
    if (key === 'calc') {
      setMode('MENU');
      setSubFunction('CALCULATOR');
      setCalcDisplay('0');
      setCalcOperand1(null);
    ...
    if (['+', '-', '*', '/'].includes(key)) {
      if (subFunction === 'CALCULATOR') {
        ...
        if (calcOperand1 === null) {
          setCalcOperand1(inputValue);
        } else if (calcOperator) {
          const result = performCalc(calcOperand1, inputValue, calcOperator);
          setCalcDisplay(String(result));
          setCalcOperand1(result);
        }
    ...
    if (key === 'clr') {
      ...
      } else if (subFunction === 'CALCULATOR') {
        setCalcDisplay('0');
        setCalcOperand1(null);
        setCalcOperator(null);
        setCalcWaitingForOperand(false);
  ...
  // Função auxiliar para cálculos da calculadora
  ...
  const renderScreen = () => {
      ...
      return (
        <div className="flex flex-col h-full justify-center items-center">
          ...
          <div className="mt-4 text-[10px] text-gray-600 flex flex-col items-center gap-1">
            ...
            <span>{t.buttons.clr}: {lang === 'PT' ? 'Zerar' : 'Reset'}</span>
          </div>
        </div>
      );
    }
    ...
    if (subFunction === 'CALCULATOR') {
      return (
        <div className="flex flex-col h-full justify-center items-center">
          <div className="text-xs font-bold text-gray-500 mb-2">{t.calculator}</div>
          <div className="w-full px-2">
            {calcOperator && (
              <div className="text-right text-xs text-gray-600 font-mono mb-1">
                {calcOperand1} {calcOperator}
              </div>
            )}
            <div className="text-right text-3xl font-mono font-bold text-gray-900 bg-black/5 rounded px-2 py-1 border border-gray-600/20">
              {calcDisplay}
            </div>
          </div>
          <div className="mt-4 text-[9px] text-gray-600 text-center">
            {lang === 'PT' ? 'Use +, -, ×, ÷' : 'Use +, -, ×, ÷'}<br/>
            {t.buttons.enter}: {lang === 'PT' ? 'Calcular' : 'Calculate'}<br/>
            {t.buttons.clr}: {lang === 'PT' ? 'Limpar' : 'Clear'}
          </div>
        </div>
      );
    }

    if (mode === 'MENU' && subFunction === 'NONE') {
      return (
    ...
    return (
      <div className="flex flex-col h-full">
          ...
          {labels.map((label, idx) => (
            label && (
              <div 
                ...
                className={`flex justify-between items-center border-b border-gray-600/20 px-1 py-0.5 cursor-pointer ${currentInputIndex === idx ? 'bg-black/10' : ''}`}
              >
                <span className="text-[9px] font-bold text-gray-700 uppercase leading-tight">{label}:</span>
        ...
        {/* Área de Resultado */}
        <div className="mt-1 border-t-2 border-gray-700 pt-1 bg-black/5 rounded px-1 h-16 min-h-[4rem] flex flex-col justify-end relative flex-shrink-0">
          <div className="absolute inset-0 overflow-y-auto p-1 scrollbar-thin scrollbar-thumb-gray-500 scrollbar-track-transparent">
            <pre className="text-xs font-bold text-black whitespace-pre-wrap leading-tight text-right">
              {result || t.pressEnter}
            </pre>
          </div>
        </div>
    ...
    <div className="flex items-center justify-center p-4 md:p-8 bg-slate-100 min-h-[80vh]">
      <div className="relative w-[340px] h-[680px] bg-gray-900 rounded-[36px] shadow-2xl border-4 border-gray-800 flex flex-col p-5 select-none ring-1 ring-white/10">
        ...
        <div className="grid grid-cols-4 gap-2 mb-4">
          {/* Linha 1 */}
          <E6BButton label={t.buttons.calc} onClick={() => handlePress('calc')} color="gray" />
