...
export async function POST(
  ...
  try {
    ...
    const result = await pool.query(
      ...
      RETURNING *`,
      ['aircraft', id, sender_id, seller_id, name, email, phone, message]
    );

    // Increment inquiries count on listing
    await pool.query(
      'UPDATE aircraft_listings SET inquiries_count = inquiries_count + 1 WHERE id = $1',
      [id]
    );

    // TODO: Send email notification to seller (integrate with Resend)
...
export async function GET(
  ...
  try {
    ...
    const result = await pool.query(
      ...
      WHERE i.listing_type = 'aircraft' AND i.listing_id = $1
