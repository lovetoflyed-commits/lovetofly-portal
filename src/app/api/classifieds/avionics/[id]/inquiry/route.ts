...
export async function POST(
  ...
  try {
    ...
    const listingResult = await pool.query(
      'SELECT user_id FROM avionics_listings WHERE id = $1',
      [id]
    );
    ...
    const result = await pool.query(
      ...
      RETURNING *`,
      ['avionics', id, sender_id, seller_id, name, email, phone, message]
    );
...
export async function GET(
  ...
  try {
    ...
    const result = await pool.query(
      ...
      WHERE i.listing_type = 'avionics' AND i.listing_id = $1
