...
// POST - Upload photo to avionics listing
export async function POST(
  ...
  try {
    ...
    const listingCheck = await pool.query(
      'SELECT id FROM avionics_listings WHERE id = $1',
      [listingId]
    );
    ...
    const photoCount = await pool.query(
      `SELECT COUNT(*) as count FROM classified_photos 
       WHERE listing_type = 'avionics' AND listing_id = $1`,
      [listingId]
    );
  ...
  try {
    ...
    const photo = await pool.query(
      `SELECT photo_data, mime_type, file_name
       FROM classified_photos
       WHERE id = $1 AND listing_type = 'avionics' AND listing_id = $2`,
      [photoId, listingId]
    );
  ...
  try {
    ...
    const result = await pool.query(
      `DELETE FROM classified_photos 
       WHERE id = $1 AND listing_type = 'avionics' 
       RETURNING id`,
      [photoId]
    );
