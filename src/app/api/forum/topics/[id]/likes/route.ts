...
export async function POST(
  ...
  try {
    ...
    const existing = await pool.query(
      `SELECT id FROM forum_topic_likes WHERE topic_id = $1 AND user_id = $2`,
      [id, userId]
    );

    if (existing.rowCount > 0) {
      await pool.query(
        `DELETE FROM forum_topic_likes WHERE topic_id = $1 AND user_id = $2`,
        [id, userId]
      );
    } else {
      await pool.query(
        `INSERT INTO forum_topic_likes (topic_id, user_id) VALUES ($1, $2)`,
        [id, userId]
      );
    }

    const countResult = await pool.query(
      `SELECT COUNT(*)::int as likes_count FROM forum_topic_likes WHERE topic_id = $1`,
      [id]
    );
