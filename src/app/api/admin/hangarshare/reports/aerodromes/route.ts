...
export async function GET(request: Request) {
  try {
    ...
    if (icao) {
      filters.push(`COALESCE(hl.icao_code, '') ILIKE ${addParam(`%${icao}%`)}`);
    }
    ...
    const mainQuery = `
      ${baseCTE}
      SELECT
        COALESCE(fl.icao_code, '—') as icao_code,
        COALESCE(fl.aerodrome_name, '—') as aerodrome_name,
      ...
      GROUP BY fl.icao_code, fl.aerodrome_name, fl.city, fl.state
    ...
    const summaryQuery = `
      ${baseCTE}
      SELECT
        COUNT(DISTINCT fl.icao_code) as aerodromes,
    ...
    console.error('Error generating aerodrome report:', err);
    return NextResponse.json(
      { message: 'Error generating aerodrome report', error: err },
      { status: 500 }
    );
  }
}
